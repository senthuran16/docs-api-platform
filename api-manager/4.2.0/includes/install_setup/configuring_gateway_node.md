...
4.  Enable JSON Web Token (JWT) if required.
     ...
     For instructions, see [Generating JSON Web Token](/api-platform/docs/api-manager/4.2.0/deploy-and-publish/deploy-on-gateway/api-gateway/passing-enduser-attributes-to-the-backend-via-api-gateway/).   
...
5.  Add the public certificate of the private key (that is used for signing the tokens) to the truststore under the `"gateway_certificate_alias"` alias. 
    ...
    For instructions, see [Create and import SSL certificates](/api-platform/docs/api-manager/4.2.0/install-and-setup/setup/security/configuring-keystores/keystore-basics/creating-new-keystores/).
...
6.  Follow the steps given below to configure High Availability (HA) for the API-M Gateway:
    ...
    2.  Configure a load balancer fronting the two Gateway nodes in your deployment.
        ...
        For instructions, see [Configuring the Proxy Server and the Load Balancer](/api-platform/docs/api-manager/4.2.0/install-and-setup/setup/setting-up-proxy-server-and-the-load-balancer/configuring-the-proxy-server-and-the-load-balancer/).
