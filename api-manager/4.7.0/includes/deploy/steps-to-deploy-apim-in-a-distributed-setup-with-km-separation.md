...
For information, see [Installing and Configuring the Databases](/api-platform/docs/api-manager/4.7.0/install-and-setup/setup/setting-up-databases/overview/).
...
For more information, see [Production Deployment Guidelines](/api-platform/docs/api-manager/4.7.0/install-and-setup/setup/deployment-best-practices/production-deployment-guidelines/#production-deployment-checklist).

!!! Note
    Configure the same encryption key on all API Manager instances before starting any node for the first time so they can decrypt shared registry resources consistently. For more information, see [Configuring Encryption Key](/api-platform/docs/api-manager/4.7.0/install-and-setup/setup/security/encryption/symmetric-encryption/#generate-a-secret-key).
...
For more information, see [Creating SSL Certificates](/api-platform/docs/api-manager/4.7.0/install-and-setup/setup/security/configuring-keystores/keystore-basics/creating-new-keystores/).
...
See the instructions on [configuring the Gateway](/api-platform/docs/api-manager/4.7.0/monitoring/api-analytics/moesif-analytics/moesif-integration-guide/) with the cloud-based analytics solution.
...
3. If required, encrypt the Auth Keys (access tokens, client secrets, and authorization codes), see [Encrypting OAuth Keys](/api-platform/docs/api-manager/4.7.0/api-security/key-management/tokens/encrypting-oauth2-tokens/).
...
4. Add the following configuration to set a unique identifier for each Gateway node when setting up a distributed deployment on VMs.
    ...
    !!! Info
        To further optimize the gateway notification feature, you can use additional `gateway_notification` configurations. For more information, see [API-M Revision Deployment Monitoring](/api-platform/docs/api-manager/4.7.0/reference/config-catalog/#api-m-gateway-notification-configurations).

5. Enable JSON Web Token (JWT) if required. For instructions, see [Generating JSON Web Token](/api-platform/docs/api-manager/4.7.0/api-gateway/passing-enduser-attributes-to-the-backend-via-api-gateway/).

6. Add the public certificate of the private key (that is used for signing the tokens) to the truststore under the "gateway_certificate_alias" alias. For instructions, see [Create and import SSL certificates](/api-platform/docs/api-manager/4.7.0/install-and-setup/setup/security/configuring-keystores/keystore-basics/creating-new-keystores/).
