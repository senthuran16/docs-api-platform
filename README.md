...
pattern as `router_api-manager`, applied to AI Workspace.

Today, infra has to configure a separate gateway rule for every single
`ai-workspace-<version>` deployment. This is a tiny nginx deployment that sits in
front of all of them - infra points ONE rule (`/ai-workspace`) at this
...
Adding a new AI Workspace version later means editing `nginx.conf` here
...
- The default-version manifest block (`/ai-workspace/product-nav-manifest.json`)
