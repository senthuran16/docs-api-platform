...
pattern as `router_api-manager`, applied to API Gateway.

Today, infra has to configure a separate gateway rule for every single
`api-gateway-<version>` deployment. This is a tiny nginx deployment that sits in
front of all of them - infra points ONE rule (`/api-gateway`) at this
...
Versions covered: `1.2.0`, `1.1.0`, `1.0.0`, `next` (default: `1.2.0`).

Adding a new API Gateway version later means editing `nginx.conf` here
...
- The default-version manifest block (`/api-gateway/product-nav-manifest.json`)
